...
",(SELECT USER_DISPLAYNAME FROM USERACCOUNTS WHERE User_id = WeigherIn)" &
",(SELECT USER_DISPLAYNAME FROM USERACCOUNTS WHERE User_id = WeigherOUt)" &
",(SELECT USER_DISPLAYNAME FROM USERACCOUNTS WHERE User_id = MODBy)" &
...
#Region "SAVING DATA PROCEDURE"
    ...
    ",[CommodityCode],[DRNo],[TicketNo]" &
    ",[Gross_Wt],[Remarks],[WeigherIn],[DriverName])" &
     "VALUES " &
    "('" & INBOUND_REFNO & "','" & Now & "','" & TxtPlateNo.Text.Trim & "','" & CustCode & "','" & SupCode & "'" &
    ",'" & CommCode & "','" & TxtDrNo.Text.Trim & "','" & TxtTicketNo.Text.Trim & "'" &
        ...
        Select Case WeighIn
            Case True
                If (SysSettings.PrintAll = True) Then
                    MessageBox.Show("Please change your ticket printing settings to continue.", "", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    Return
                Else
                    ...
                    MOD_REPORTING.PrintToPrinter(TicketTypeEnum.TicketIn, TxtRefNo.Text)
                End If
            Case False
                If MessageBox.Show("Re print selected record?", "Re-Print", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.No Then Exit Sub
                MOD_REPORTING.PrintToPrinter(TicketTypeEnum.TicketAll, TxtRefNo.Text)
        End Select
