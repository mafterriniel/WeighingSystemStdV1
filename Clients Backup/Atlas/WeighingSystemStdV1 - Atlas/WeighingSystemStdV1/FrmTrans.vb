        ...
        With cls.DataReader
            If .HasRows Then
                While .Read
                    ...
                    If IsDBNull(.Item("DTIn")) Then DTIN_Tmp = "" Else DTIN_Tmp = Format(.Item("DTIn"), "MMM-dd-yyyy hh:mm tt")
...
",[DriverName] = '" & Trim(TxtDriver.Text) & "'" &
 " where RefNo = '" & RefNO_Tmp & "'"
            ...
            If SysSettings.EnablePrintOut = True Then
                Select Case SysSettings.PrintAll
                    Case True
                        MOD_REPORTING.PrintToPrinter(TicketTypeEnum.TicketAll, RefNO_Tmp, FrmLogin.UserDisplayName)
                    Case False
                        MOD_REPORTING.PrintToPrinter(TicketTypeEnum.TicketOut, RefNO_Tmp, FrmLogin.UserDisplayName)
                End Select
            End If
...
#End Region
    ...
    Private Sub BtnPrintIN_Click_1(sender As Object, e As EventArgs)

    End Sub

    Private Sub FrmTrans_Closing(sender As Object, e As CancelEventArgs) Handles Me.Closing

    End Sub

    Private Sub FrmTrans_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        TxtOnline.Stop()
        TxtOnline.Dispose()
        CLS_CLEARMEM.ClearMem(New Object() {TxtOnline})
    End Sub

    Private Sub PushButton1_Click(sender As Object, e As EventArgs) Handles PushButton1.Click
        TxtOnline.Stop()
        TxtOnline.Start()
    End Sub
End Class